...
from model.module.util import cross_view_cls_inner_product, ReduceDim
import torch as th
...
class JPQ(BaseModel):
  ...
  SIGIR'21 - Jointly Optimizing Query Encoder and Product Quantization to Improve Retrieval Performance
  """
  ...
  def forward(self,
    ...
    quant_text_cls_embd, _, _, normalized_text_cls_embd, quant_text_cls_reg = self.cls_pq_layer(text_cls_embd)
    quant_text_cls_reg = quant_text_cls_reg.mean(0)  # B x Caps -> Caps
    quant_vid_cls_embd, _, _, normalized_vid_cls_embd, quant_vid_cls_reg = self.cls_pq_layer(vid_cls_embd)
    quant_vid_cls_reg = quant_vid_cls_reg.mean(0, keepdim=True)  # B -> 1
    quant_reg = th.cat([quant_text_cls_reg, quant_vid_cls_reg])  # -> (Caps+1)
    quant_reg = quant_reg.mean()  # (Caps+1) -> ()
    ...
    if out == 'conf':  # Output confusion matrix
      ...
      with th.no_grad():
        ...
        quant_err = (quant_text_cls_err + quant_vid_cls_err) / 2  # -> ()
