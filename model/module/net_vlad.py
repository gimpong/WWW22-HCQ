...
class NetVLAD(nn.Module):
  ...
  def __init__(self, cluster_size, feature_size, ghost_cluster_size=1, add_batch_norm=True):
    super().__init__()

    self.feature_size = feature_size
    self.cluster_size = cluster_size
    self.ghost_cluster_size = ghost_cluster_size
    init_sc = (1 / math.sqrt(feature_size))
    # The `clusters` weights are the `(w,b)` in the paper
    self.clusters = nn.Parameter(init_sc * th.randn(feature_size, cluster_size + ghost_cluster_size))
  ...
  def forward(self, x, x_mask=None):
    ...
    feature_size, G = num_ghost_clusters.

    Args:
      x: B x N x D
      x_mask [boolean]: B x N
    ...
    # fliter out the ghost clusters
    assignment = assignment[:, :self.cluster_size] # BN x (K+G) -> BN x K
    # mask and take the valid tokens in the sequence
    if x_mask is not None:
      mask = x_mask.to(th.float).view(-1, 1) # BN x 1
    ...
    # L2 intra norm # 2021/12/13 removed
    # vlad = F.normalize(vlad) # -> B x D x K

    vlad = vlad.transpose(1, 2)  # -> B x K x D
    
    return vlad  # B x K x D

    # MASED 2021/12/12
    # flattening + L2 norm
    # vlad = vlad.reshape(-1, self.cluster_size * self.feature_size)  # B x D x K -> B x DK
