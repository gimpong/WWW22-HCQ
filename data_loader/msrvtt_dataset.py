...
class MSRVTT(BaseDataset):
  ...
  def configure_train_test_splits(self, cut_name, split_name):
    ...
    if cut_name in ["miech", "jsfusion"]:
      ...
      # 2021/12/23 rewrited
      self.vid_list = []
      if split_name in ["trainval", "trnval"]: 
        list_path = "train_list_full.txt"
        with open(os.path.join(self.data_dir, "val_list_full.txt")) as f:
          self.vid_list += f.readlines()
        with open(os.path.join(self.data_dir, "test_list_full.txt")) as f:
          nb_val_samples = len(f.readlines())
      elif split_name in ["train", "trn"]:
        list_path = "train_list_full.txt"
        with open(os.path.join(self.data_dir, "val_list_full.txt")) as f:
          nb_val_samples = len(f.readlines())
