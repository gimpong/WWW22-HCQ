...
"""Training process for Hybrid Contrastive Quantization.
...
from model.module.util import cross_view_cls_inner_product, cross_view_vlad_inner_product, \
    sharded_cross_view_cls_inner_product, sharded_cross_view_vlad_inner_product
import numpy as np
...
class HCQTrainer(BaseTrainer):
  ...
  def _train_epoch(self, epoch, scaler):
    ...
    if self.update_codebooks_by_clustering:
      ...
      text_cls_embd_cache = []
      vid_cls_embd_cache = []
      text_vlad_embds_cache = []
      vid_vlad_embds_cache = []
      ...
      with autocast():
        ...
        if out == "conf":
          loss = self.loss(quant_cls_t2v_sims=output['quant_vid_t2v_cls_conf_matrix'],  # -> B x B
                          ...
                          quant_vlad_t2v_sims=output['quant_vid_t2v_vlad_conf_matrix'],  # -> Kvlad x B x B
                          quant_vlad_v2t_sims=output['quant_text_t2v_vlad_conf_matrix'].transpose(-1,-2),
                          feat_cls_t2v_sims=output['t2v_cls_conf_matrix'],  # -> B x B
                          ...
                          code_cls_t2v_sims=output['quant_code_t2v_cls_conf_matrix'],  # -> M x B x B
                          code_cls_v2t_sims=output['quant_code_t2v_cls_conf_matrix'].transpose(-1,-2), 
                          code_vlad_t2v_sims=output['quant_code_t2v_vlad_conf_matrix'],  # -> Kvlad x M x B x B
                          code_vlad_v2t_sims=output['quant_code_t2v_vlad_conf_matrix'].transpose(-1,-2), 
                          epoch=epoch)
          ...
          if self.update_codebooks_by_clustering:
            ...
            text_vlad_embds_cache.append(output['text_vlad_embds'].detach())
            vid_vlad_embds_cache.append(output['vid_vlad_embds'].detach())
          ...
          quant_vid_t2v_vlad_conf_matrix = cross_view_vlad_inner_product(
            ...
            vid_vlad_embds=output['quant_vid_vlad_embds'],
            merge_caption_similarities=self.model.merge_caption_similarities
          )
          # -> Kvlad x B x B
          quant_text_t2v_vlad_conf_matrix = cross_view_vlad_inner_product(
            text_vlad_embds=output['quant_text_vlad_embds'],
          ...
          quant_code_t2v_cls_conf_matrix = sharded_cross_view_cls_inner_product(
            text_cls_code=output['quant_text_cls_code'],
            vid_cls_code=output['quant_vid_cls_code'],
            merge_caption_similarities=self.model.merge_caption_similarities
          )
          # -> Kvlad x M x B x B
          quant_code_t2v_vlad_conf_matrix = sharded_cross_view_vlad_inner_product(
            text_vlad_codes=output['quant_text_vlad_codes'],
            vid_vlad_codes=output['quant_vid_vlad_codes'],
            merge_caption_similarities=self.model.merge_caption_similarities
          )
          ...
          with torch.no_grad():
            ...
            # B x Caps x K x D -> K
            quant_text_vlad_errs = F.mse_loss(output["text_vlad_embds"], 
                                              output["quant_text_vlad_embds"], 
                                              reduction='none').sum(-1).mean(-2).mean(0)
            # B x K x D -> K
            quant_vid_vlad_errs = F.mse_loss(output["vid_vlad_embds"], 
                                            output["quant_vid_vlad_embds"], 
                                            reduction='none').sum(-1).mean(0)
            quant_vlad_errs = (quant_text_vlad_errs + quant_vid_vlad_errs) / 2  # -> K
            quant_err = torch.cat([quant_cls_err, quant_vlad_errs], dim=-1).mean(-1)  # (K+1) -> ()
          ...
          loss = self.loss(quant_cls_t2v_sims=quant_vid_t2v_cls_conf_matrix,  # -> B x B
                          ...
                          quant_vlad_t2v_sims=quant_vid_t2v_vlad_conf_matrix,  # -> Kvlad x B x B
                          quant_vlad_v2t_sims=quant_text_t2v_vlad_conf_matrix.transpose(-1,-2),
                          feat_cls_t2v_sims=t2v_cls_conf_matrix,  # -> B x B
                          ...
                          code_cls_t2v_sims=quant_code_t2v_cls_conf_matrix,  # -> M x B x B
                          code_cls_v2t_sims=quant_code_t2v_cls_conf_matrix.transpose(-1,-2),
                          code_vlad_t2v_sims=quant_code_t2v_vlad_conf_matrix,  # -> Kvlad x M x B x B
                          code_vlad_v2t_sims=quant_code_t2v_vlad_conf_matrix.transpose(-1,-2),
                          epoch=epoch)
    ...
    if self.update_codebooks_by_clustering:
      ...
      self.model.update_codebooks(text_cls_embd=torch.cat(text_cls_embd_cache),
                                  ...
                                  text_vlad_embds=torch.cat(text_vlad_embds_cache),
                                  vid_vlad_embds=torch.cat(vid_vlad_embds_cache))
  ...
  def log_metrics(self, metric_store, epoch, metric_name, dataset_name):
    for key, value in metric_store.items():
      if key != "cols":
        self.writer.add_scalar(f"{dataset_name}/{metric_name}/{key}", value, epoch)
  ...
  def _get_embeddings(self, val_loader):
    ...
    with torch.no_grad():
      ...
      quant_text_vlad_embds_list = []
      quant_vid_vlad_embds_list = []
      quant_reg_list = []
      query_masks_list = []
      raw_captions_list = []
      token_ids_list = []
      ...
      for batch_idx, minibatch in enumerate(val_loader):
        ...
        quant_text_vlad_embds_list.append(output['quant_text_vlad_embds'])
        quant_vid_vlad_embds_list.append(output['quant_vid_vlad_embds'])
      ...
      quant_text_vlad_embds = torch.cat(quant_text_vlad_embds_list)
      quant_vid_vlad_embds = torch.cat(quant_vid_vlad_embds_list)
    ...
    with torch.no_grad():
      ...
      for dataset_name, embds in loaders_embds.items():
        ...
        quant_vid_t2v_vlad_conf_matrix = cross_view_vlad_inner_product(
          ...
          vid_vlad_embds=embds['quant_vid_vlad_embds'],
          merge_caption_similarities=self.model.merge_caption_similarities
        )
        quant_vid_t2v_sims = self.model.cls_sims_weight * quant_vid_t2v_cls_conf_matrix + \
          self.model.vlad_sims_weight * quant_vid_t2v_vlad_conf_matrix.mean(0)
        ...
        quant_text_t2v_vlad_conf_matrix = cross_view_vlad_inner_product(
          text_vlad_embds=embds['quant_text_vlad_embds'],
          vid_vlad_embds=embds['vid_vlad_embds'],
          merge_caption_similarities=self.model.merge_caption_similarities
        )
        quant_text_t2v_sims = self.model.cls_sims_weight * quant_text_t2v_cls_conf_matrix + \
          self.model.vlad_sims_weight * quant_text_t2v_vlad_conf_matrix.mean(0)
