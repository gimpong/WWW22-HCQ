...
"""Training process for Deep Cross-Modal Hashing.
...
class DCMHTrainer(BaseTrainer):
  ...
  def _train_epoch(self, epoch, scaler):
    ...
    for batch_idx, minibatch in enumerate(self.train_loaders[i]):
      ...
      with autocast():
        # if True:
        output = self.model(**minibatch, device=self.device, debug=debug)

        self.timer.update("train_batch.forward", time.time() - forward_start)
        loss_start = time.time()
        loss, loss_val_dict = self.loss(text_cls_embd=output['text_cls_embd'], 
      ...
      if batch_idx % self.log_step == 0:
        ...
        msg = (f"Train Epoch: {epoch} {prog} "
               ...
               f"(semantic_loss: {loss_val_dict['semantic_loss']:.5f}, "
               f"quant_loss: {loss_val_dict['quant_loss']:.5f}, "
               f"bit_balance_loss: {loss_val_dict['bit_balance_loss']:.5f}) "
  ...
  def _valid_epoch(self, epoch=None, sets="continuous_eval"):
    ...
    with torch.no_grad():
      ...
      for dataset_name, embds in loaders_embds.items():
        ...
        t2v_sims = cross_view_cls_inner_product(
        ...
        if sets == "final_eval":
          ...
          if cut_name == "c" and split_name in ["test1", "test2"]:
            ...
            prediction_path = pathlib.Path(
                self.exp_dir) / f"{dataset_basename}-{split_name}-predictions.csv"
            ...
            np.savetxt(prediction_path,
                       ...
                       fmt="%d")
            logger.debug(
                "Saved v2t similarity matrix predictions to %s", prediction_path)
          sims_data = {"t2v_sims": t2v_sims, "query_masks": query_masks}
          sims_path = pathlib.Path(self.exp_dir) / \
              f"{dataset_basename}-{split_name}-sims.npy"
        ...
        for metric in self.metrics:
          ...
          nested_metrics[metric_name] = metric(
              t2v_sims, query_masks=query_masks)
