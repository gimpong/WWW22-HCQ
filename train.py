...
import torch
from trainer import HCQTrainer, HCTTrainer, DCMHTrainer, JPQTrainer
...
def train(config):
  ...
  tokenizer = create_tokenizer(config["arch"]["args"]["txt_inp"], config["arch"]["args"].get("txt_bert_type", 'bert-base'))
  ...
  assert config["arch"]["type"] in ['HCQ', 'HCT', 'DCMH', 'JPQ']
  if config["arch"]["type"] == 'HCQ':
    trainer = HCQTrainer(
        model,
        loss,
        metrics,
        ...
        codebook_fixed_epoches=config["trainer"].get("codebook_fixed_epoches", 0),
        update_codebooks_by_clustering=config["trainer"].get("update_codebooks_by_clustering", False),
        include_optim_in_ckpts=config["trainer"].get("include_optim_in_ckpts", False),
        expert_dims=expert_dims,
        tokenizer=tokenizer,
        warmup_iterations=warmup_iterations)
  elif config["arch"]["type"] == 'HCT':
    trainer = HCTTrainer(
        model,
        loss,
        metrics,
    ...
    raise NotImplementedError("undefined model architecture")
