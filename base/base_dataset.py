...
class BaseDataset(Dataset):
  ...
  def __init__(self,
    ...
    self.cache_dir = os.path.join(os.path.dirname(data_dir), "vid_feat_files", self.reading_from)
    ...
    if self.load_in_ram:
      logger.info(f"Loading dataset {self.dataset_name} in ram ...")
      begin_loading_time = time.time()
      if self.reading_from == "mult_h5":
        self.data_vid = {}
        for idx, vid in enumerate(self.vid_list):
          if idx % 100 == 0:
            logger.debug(idx)
          self.data_vid[vid] = self.get_sample_data(vid)
      ...
      logger.info(f"Finish loading dataset {self.dataset_name} in ram, taking {end_loading_time-begin_loading_time} s.")
  ...
  def get_sample_data(self, vid):
    if self.loaded_in_ram:
      return deepcopy(self.data_vid[vid])
    ...
    if self.reading_from == "mult_h5":
      ...
      with h5py.File(dataset_file_path, "r") as dataset_file:
        ...
        for i in range(nb_captions):
          raw_caption = video_data[f"raw_captions.{i}"][()]
          raw_captions.append(raw_caption)
          if f"raw_captions_t.{i}" in video_data.keys():
            raw_caption_t = video_data[f"raw_captions_t.{i}"][()]
        ...
        for expert in self.experts:
          if f"features.{expert}" in video_data.keys():
            x = video_data[f"features.{expert}"][()]
            if len(x) > 0 and not np.isnan(x[0][0]):
              features[expert] = video_data[f"features.{expert}"][()]

              if f"features_t.{expert}" in video_data.keys():
                x = video_data[f"features_t.{expert}"][()]
  ...
  def __getitem__(self, idx):
    ...
    for _ in range(self.n_pairs):
      ...
      for cap_idx in range(self.captions_per_video):
        ...
        raw_captions_.append(selected_sentences)  # [cap_per_vid * np.array(["aa", "bb"])]
      ...
      captions_token_ids = np.stack(token_ids, axis=0)  # (captions_per_video, max_text_words, 2)
      ...
      for expert in self.experts:
        ...
        if expert in features.keys():
          if clip_length == float("inf"):
            ...
            # 2s.
            features_t_sel = (features_t[expert] - feat_start) / self.temporal_encoding_window
            features_t_sel = features_t_sel + 2
          else:
            indices_to_keep = np.logical_and(feat_start <= features_t[expert], features_t[expert] <= feat_end)
            nb_kept = indices_to_keep.sum()
            if nb_kept > 0:
              features_sel = features[expert][indices_to_keep]
              # For temporal encoding, we want the temporal features to start at
              # 2s.
              features_t_sel = (features_t[expert][indices_to_keep] - feat_start) / self.temporal_encoding_window
              features_t_sel = features_t_sel + 2
            else:
    ...
    features_avgpool = {}
    features_maxpool = {}
    token_ids = np.stack(token_ids_list, axis=0)  # (n_pairs, captions_per_video, max_text_words, 2)
    query_masks = np.stack(query_masks_list, axis=0)  # (n_pairs, captions_per_video)
    for expert in self.experts:
      features_list = features_dic[expert]  # [n_pairs x (max_expert_tokens, feat_dim)]
      features[expert] = np.stack(features_list, axis=0)  # (n_pairs, max_expert_tokens, feat_dim)
      features_t_list = features_t_dic[expert]  # [n_pairs x (max_expert_tokens)]
      features_t[expert] = np.stack(features_t_list, axis=0)  # (n_pairs, max_expert_tokens)
      features_ind_list = features_ind_dic[expert]  # [n_pairs x (max_expert_tokens)]
      features_ind[expert] = np.stack(features_ind_list, axis=0)  # (n_pairs, max_expert_tokens)
      features_avgpool[expert] = np.concatenate(features_avgpool_dic[expert], axis=0)  # (n_pairs, feat_dim)
      features_maxpool[expert] = np.concatenate(features_maxpool_dic[expert], axis=0)  # (n_pairs, feat_dim)
